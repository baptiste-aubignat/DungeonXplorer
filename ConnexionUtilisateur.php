...
if (isset($_POST['envoiConnexion'])) {
    if (
        !empty($_POST['password']) and !empty($_POST['pseudoMail'])
    ) {
        $pseudo = htmlspecialchars($_POST['pseudoMail']);
        $password = sha1($_POST['password']);

        $sth = 'SELECT name, email, password FROM Account;';
        foreach ($conn->query($sth) as $row) {

            if ($pseudo == $row['name'] or $pseudo == $row['email']) {
        ...
        echo "Adresse mail ou pseudo inexistant";
    } else {
                    ...
                    <div class="field">
                        ...
                        <div class="control has-icons-left">
                            ...
                            <div id="seudoOuMailValide">
                                <input name="pseudoMail" class="input" type="text" placeholder="Pseudo ou mail">
                            </div>
